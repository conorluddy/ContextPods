...
import type { CommandContext, CommandResult, ServerOptions, ServerStatus } from '../types/cli-types.js';
...
export async function statusServerCommand(
  ...
  try {
    ...
    const serverPath = join(context.workingDir, 'packages/server/dist/index.js');
    status.built = await fs.access(serverPath).then(() => true).catch(() => false);
    ...
    status.configured = await fs.access(configPath).then(() => true).catch(() => false);
...
export async function testServerCommand(
  ...
  try {
    ...
    const testExists = await fs.access(testScript).then(() => true).catch(() => false);
...
export async function devServerCommand(
  ...
  try {
    ...
    const devExists = await fs.access(devScript).then(() => true).catch(() => false);
