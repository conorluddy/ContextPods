...
import { WrapOptions, CommandContext, CommandResult } from '../types/cli-types.js';
...
function detectLanguage(ext: string, content: string): string {
  switch (ext) {
    case '.js':
    case '.mjs':
      return content.includes('import ') || content.includes('export ') ? 'javascript-es6' : 'javascript';
    case '.ts':
      return 'typescript';
    case '.py':
      return 'python';
...
function displayAnalysis(analysis: ScriptAnalysis): void {
  ...
  output.table([
    ...
    { label: 'Complexity', value: analysis.complexity, color: analysis.complexity === 'simple' ? 'green' : analysis.complexity === 'moderate' ? 'yellow' : 'red' },
    { label: 'Has Exports', value: analysis.hasExports ? 'Yes' : 'No', color: analysis.hasExports ? 'green' : 'gray' },
    { label: 'Has Imports', value: analysis.hasImports ? 'Yes' : 'No', color: analysis.hasImports ? 'green' : 'gray' },
