...
export async function listCommand(
  ...
  try {
    ...
    const filteredMCPs = options.all 
      ? mcps 
      : mcps.filter(mcp => mcp.status !== 'inactive');
  ...
  try {
    ...
    try {
      ...
      if (packageJson.keywords?.includes('mcp') || 
        ...
        template = packageJson['context-pods']?.template || 
      ...
      const hasIndexFile = await pathExists(path.join(mcpPath, 'src', 'index.ts')) ||
...
async function detectTemplateFromStructure(mcpPath: string): Promise<string | undefined> {
  ...
  // Check for Python
  if (await pathExists(path.join(mcpPath, 'requirements.txt')) ||
...
function displayMCPTable(mcps: MCPInfo[]): void {
  ...
  mcps.forEach((mcp, index) => {
    ...
    console.log(`${index + 1}. ${output.package(mcp.name)}`);
  ...
  const statusCounts = mcps.reduce((acc, mcp) => {
