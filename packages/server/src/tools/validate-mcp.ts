...
export class ValidateMCPTool extends BaseTool {
  ...
  private async validateBasicStructure(mcpPath: string, result: ValidationResult): Promise<void> {
    try {
      ...
      result.info.hasMainFile = mainFiles.some(file => files.includes(file) || files.includes('src'));

      if (!result.info.hasPackageJson && !result.info.hasMainFile) {
        result.errors.push('No recognizable MCP server structure found (missing package.json or main entry file)');
      }
      ...
      result.errors.push(`Failed to read directory structure: ${error}`);
    }
  }
  ...
  private async validatePackageJson(mcpPath: string, result: ValidationResult): Promise<void> {
    ...
    try {
      ...
      if (packageJson.type !== 'module' && packageJson.type !== 'commonjs') {
        result.warnings.push('Consider setting "type" field in package.json ("module" or "commonjs")');
      }
  ...
  private async validateMainFileContent(filePath: string, result: ValidationResult): Promise<void> {
    try {
      ...
      result.errors.push(`Failed to validate main file content: ${error}`);
    }
  }
    ...
    try {
      ...
      if (server) {
        ...
        if (server.status === 'error') {
          result.warnings.push(`Server is registered but has error status: ${server.metadata.errorMessage || 'Unknown error'}`);
        }
      } else {
      ...
      result.warnings.push(`Failed to check registry status: ${error}`);
    }
  }
  ...
  private async validateMCPSchema(mcpPath: string, result: ValidationResult): Promise<void> {
    ...
    try {
      ...
      result.warnings.push(`Schema validation incomplete: ${error}`);
    }
  }
  ...
  private async validateBuild(mcpPath: string, result: ValidationResult): Promise<void> {
    ...
    try {
      ...
      result.warnings.push(`Build validation failed: ${error}`);
    }
  }
